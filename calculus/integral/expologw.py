	...
	if inputexpression[:4]=='log(':
		if inputexpression[4:4+len(dvar)]==dvar:
			if inputexpression[4+len(dvar):]==')':
				return [True,[['('+dvar+')*ln('+dvar+')-'+dvar,'Correct',.9],['1/('+dvar+')','Take the integral not the derivative',.05],['('+dvar+')*ln('+dvar+')','Do not forget to subtract '+ dvar,.05]], 'Apply the logarithm rule.']
			else:
				return [False,inputexpression]
				...
				if inputexpression[:len(the_base)+2]==the_base+'^(':
					if inputexpression[len(the_base)+2:len(the_base)+2+len(dvar)]==dvar:
						if inputexpression[len(the_base)+2+len(dvar):]==')':
							if the_base == 'e':
								return [True,[[inputexpression,'Correct',.9],['e^('+dvar+'+1)/('+dvar+'+1)','The variable is in the exponent so treat the function as an exponential function.',.1]],'Apply the exponential rule.']
							else:
								return [True,[[inputexpression+'/log('+the_base+')','Correct',.9],[inputexpression,'Do not forget to divide by the logarithm of the base.',.05],[inputexpression+'*log('+the_base+')','Divide by the logarithm of the base instead of multiplying.',.05]], 'Apply the exponential rule to <div class="katex_div_il">a^'+dvar+'</div> with the base <div class="katex_div_il">a='+the_base+'</div>.']
						else:
							return [False,inputexpression]
...
def expologrulew(inputexpression,dvar):
