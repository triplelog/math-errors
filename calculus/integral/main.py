...
def fullintegral(inputexpression,dvar,ycount,wrongness,tryudv):
	...
	if h[0]!=1:
		...
		dparts = [[dvar+'('+h[0]+')','a',''],['*','b',''],[dvar+'\int('+h[1]+')','c','',fullintegral(h[1],dvar,ycount,wrongness,tryudv)[1]]]
		...
		if len(h)>1:
			...
			for idx, i in enumerate(h):
				...
				if idx %2==0:
					...
					dparts.append([dvar+'\int('+i+')',nletter,'',fullintegral(i,dvar,ycount,wrongness,tryudv)[1]])
				else:
					if i==0:
						f=f+'+'
...
dparts = fullintegral(sys.argv[1],'x',0,0,0)[1]
print dparts
