	...
	if inputexpression[:4]=='log(':
		if inputexpression[4:4+len(dvar)]==dvar:
			if inputexpression[4+len(dvar):]==')':
				return [True,'1/('+dvar+')', 'Apply the logarithm rule.',0]
			else:
...
def exporule(inputexpression,dvar):
	...
	if found_one:
		...
		if fullparen(the_base):
			spart = the_base
			for sstr in ['sin','cos','log','tan','cot','sec','csc','cot','sqrt','arc','ln','abs']:
				...
				if inputexpression[:len(the_base)+2]==the_base+'^(':
					if inputexpression[len(the_base)+2:len(the_base)+2+len(dvar)]==dvar:
						if inputexpression[len(the_base)+2+len(dvar):]==')':
							if the_base == 'e':
								return [True,inputexpression,'Apply the exponential rule.',2]
							else:
								return [True,inputexpression+'log('+the_base+')', 'Apply the exponential rule to <div class="katex_div_il">a^'+dvar+'</div> with the base <div class="katex_div_il">a='+the_base+'</div>.',3]
						else:
							return [False,inputexpression]
