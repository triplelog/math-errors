...
def solve_for_dydx(my_question):
	...
	return '('+rhs+')/('+dby+')',ycount_intl,ycount_intr
...
def get_question(q_id):
	...
	if q_id == 'One':
		...
		return 'y^2='+in_fn, 'y^2='+in_fn
	elif q_id == 'Two':
		q_1 = get_rand_poly('x')
		return 'y*ln(y)='+q_1, 'y\ln(y)='+q_1
	elif q_id == 'Three':
		q_1 = get_rand(trig_fns,[.25,.25,.15,.15,.1,.1])
		q_2 = get_rand_poly('x')
		q_1 = q_1.replace('x','y')
		return 'y'+q_1+'='+q_2,'y\\'+q_1+'='+q_2
	elif q_id == 'Four':
		if random.random()<.3:
	...
	if(form.is_valid()):
		...
		if my_question != 'LATEX':
			try:
				...
				c_my_answer = cleanpar(c_my_answer,'y')
				c_fds = cleanpar(fds,'x')
				c_fds = cleanpar(fds,'y')
...
def implicit_differentiation_worksheet(request):
	...
	rcarr = [{'href':'implicit_differentiation_lesson.html','title':'Implicit Differentiation Lesson'},{'href':'product_rule_worksheet.html','title':'Product Rule Worksheet'},{'href':'quotient_rule_worksheet.html','title':'Quotient Rule Worksheet'},{'href':'#','title':'Chain Rule Video'},{'href':'#','title':'Chain Rule Explained'}]
	meta_title = 'Implicit Differentiation Worksheet - Learn Implicit DIfferentiation by working examples with Calculus College.'
	meta_des = "Infinitely many implicit differentiation problems with step-by-step solutions if you make a mistake. Progress through several types of problems that help you improve."
	return render(request,'base_worksheet_id.html',{'related_content':rcarr,'meta-des':meta_des,'title':meta_title,'header_1':'Implicit Differentiation Worksheet','sections':vsections,'fraction_questions':frac_q})
