...
def get_question(q_id):
	...
	if q_id == 'One':
		...
		in_fn = in_fs[ch_x]
		if ch_x in [2,4]:
			return 'x^'+str(q_1)+'('+in_fn+')','x^{'+str(q_1)+'}\\'+in_fn
		else:
			return 'x^'+str(q_1)+'('+in_fn+')','x^{'+str(q_1)+'}('+in_fn+')'
	elif q_id == 'Two':
		q_1 = get_rand_poly('x')
		return '('+q_1+')ln(x)', '('+q_1+')\ln{(x)}'
	elif q_id == 'Three':
		q_n = get_rand(trig_fns,[.25,.25,.15,.15,.1,.1])
		q_d = get_rand_poly('x')

		return '('+q_d+')'+q_n,'('+q_d+')\\'+q_n
	elif q_id == 'Four':
		if random.random()<.75:
			q_d = get_rand_poly('x')
			q_1 = 'e^x('+q_d+')'
		...
		if random.random()<.4:
			...
			q_3_l = '(\\'+trig_fns[q_1]+'+\\'+trig_fns[q_2]+')x^'+q_p
			return q_3, q_3_l
		else:
			if random.random()<.5:
				q_p = str(random.randint(2,8))
				q_3 = '(e^x+x^'+q_p+')ln(x)'
				q_3_l = '(e^x+x^'+q_p+')\ln(x)'
				return q_3,q_3_l
			else:
				q_p = str(random.randint(2,8))
				q_3 = '(ln(x)+x^'+q_p+')e^x'
				q_3_l = '(\ln(x)+x^'+q_p+')e^x'
				return q_3,q_3_l
...
def product_rule_worksheet(request):
				...
				try:
					...
					for i in range(0,10):
						if new_q[0]==my_question:
							...
							break
					return any_worksheet(form,'product',new_q,my_type)
				except:
					pass
			return any_worksheet(form,'product','noq','Zero')
		else:
	...
	vsections = [{'title':'The Product Rule','subsections':[{'title':'Powers','id':q_id,'form':WorksheetForm(),'question_python':gq[0],'question_latex':gq[1],'classinit':'BB'}]}]
	...
	vsections.append({'title':'Advanced','subsections':[{'title':'Triple Product','id':q_id,'form':WorksheetForm(),'question_python':gq[0],'question_latex':gq[1],'classinit':'BB'}]})
	q_id = 'Six'; gq = get_question(q_id)
	vsections[1]['subsections'].append({'title':'Sum Rule','id':q_id,'form':WorksheetForm(),'question_python':gq[0],'question_latex':gq[1],'classinit':'BB'})
	...
	return render(request,'base_worksheet.html',{'pform':PrintableForm(),'related_content':rcarr,'title':meta_title,'meta_des':meta_des,'header_1':'Product Rule Worksheet','sections':vsections,'fraction_questions':frac_q})
