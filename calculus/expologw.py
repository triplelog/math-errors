				...
				if inputexpression[:len(the_base)+2]==the_base+'^(':
					if inputexpression[len(the_base)+2:len(the_base)+2+len(dvar)]==dvar:
						if inputexpression[len(the_base)+2+len(dvar):]==')':
							if the_base == 'e':
								return [True,[[inputexpression,'Correct'],['('+dvar+')*e^('+dvar+'-1)','Do not use the power rule for exponential functions-The derivative of <div class="katex_div_il">'+slatex(inputexpression)+'</div> is <div class="katex_div_il">'+slatex(inputexpression)+'</div> not <div class="katex_div_il">'+slatex('('+dvar+')*e^('+dvar+'-1)')+'</div>.']],'Apply the exponential rule.']
							else:
								return [True,[[inputexpression+'log('+the_base+')','Correct'],[inputexpression,'Forgot to multiply by the log of the base'],['('+dvar+')*'+the_base+'^('+dvar+'-1)','Do not use the power rule for exponential functions-The derivative of <div class="katex_div_il">'+slatex(inputexpression)+'</div> is <div class="katex_div_il">'+slatex(inputexpression+'*log('+the_base+')')+'</div> not <div class="katex_div_il">'+slatex('('+dvar+')*'+the_base+'^('+dvar+'-1)')+'</div>.']], 'Apply the exponential rule to <div class="katex_div_il">a^'+dvar+'</div> with the base <div class="katex_div_il">a='+the_base+'</div>.']
						else:
							return [False,inputexpression]
