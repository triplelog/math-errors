...
function makeGraph(input){
	...
	if (domain[1]-domain[0]>5){
		for (var i=Math.floor(domain[0])+1;i<Math.floor(domain[1])+1;i++){
			...
			if (i%5 == 0 && i != 0){
				svg += '<circle cx="'+convertX(i,domain)+'" cy="'+convertY(0,range,2)+'" r="4px" fill="white"/>';
		...
		for (var ii=Math.floor(range[0])+1;ii<Math.floor(range[1])+1;ii++){
			if (ii%5 == 0 && ii != 0){
				svg += '<text font-size="6px" text-anchor="end" dominant-baseline="middle" x="'+convertX(0,domain,-1)+'" y="'+convertY(ii,range)+'">'+ii+'</text>';
			}	
		}
	}
