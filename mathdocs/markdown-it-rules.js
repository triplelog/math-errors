...
function makeGraph(input){
	...
	svg += '<defs><filter x="0" y="0" width="1" height="1" id="solid"><feFlood flood-color="white"/><feComposite in="SourceGraphic" operator="xor"/></filter></defs>';
	...
	if (domain[1]-domain[0]>5){
		for (var i=Math.floor(domain[0])+1;i<Math.floor(domain[1])+1;i++){
			...
			if (i%5 == 0 && i != 0){
				svg += '<circle cx="'+convertX(i,domain)+'" cy="'+convertY(0,range,2)+'" r="3px" fill="red"/>';
				
				svg += '<text fill="black" font-size="6px" text-anchor="middle" dominant-baseline="hanging" x="'+convertX(i,domain)+'" y="'+convertY(0,range,2)+'">&nbsp;'+i+'&nbsp;</text>';
			}
		}
		for (var ii=Math.floor(range[0])+1;ii<Math.floor(range[1])+1;ii++){
			if (ii%5 == 0 && ii != 0){
